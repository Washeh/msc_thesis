...
# at any point, if the number of 1s in the sequence till now is even, print 0, else 1
def generate_data(count, sequence_length):
    ...
    for i in range(count):
        ...
        for j in range(sequence_length):
            bit = (bit + sequence[j]) % 2
            expected[j, 0, 0] = bit
        inputs[i] = observation
        outputs[i] = expected
    mask = np.array([[1] for i in range(sequence_length)])
...
def print_results_for(w, inputs, outputs, count=3):
    random_slice = np.random.randint(0, len(inputs), count)
    sliced_outputs = outputs[random_slice]
    sliced_inputs = inputs[random_slice]
    print('results: observation / expected / predicted', '\n')
    for i in range(count):
        observation = sliced_inputs[i]
        predicted = w.predict(observation)
        expected = sliced_outputs[i]
        results = np.zeros((3, len(observation[:, 0, 0])))
        results[0, :] = observation[:, 0, 0]
        results[1, :] = expected[:, 0, 0]
        results[2, :] = predicted[:, 0, 0]
        print(results.T, '\n')


def train(w, inputs, outputs, sequence_mask, num_iterations, report_interval):
    total_loss = 0
...
def main(unused_argv):
    ...
    with tf.Session() as sess:
        ...
        train(wrapper, inputs, outputs, mask, iterations, report_interval)
        wrapper.save()

        print_results_for(wrapper, inputs, outputs)
